...
# reflectivity defines a trivial njit if numba is unavailable
# TODO: move USE_NUMBA to util
from .reflectivity import njit, USE_NUMBA
#USE_NUMBA = 0  # suppress numba for speed tests
...
def SCFeqns(phi_z, chi, chi_s, sigma, n_avg, p_i, phi_b=0):
    ...
    u_int = 2 * chi * (correlate(phi_z, LAMBDA_ARRAY) - phi_b)
    u_int[0] += chi_s
    u_z = u_prime + u_int
    g_z = exp(u_z)
...
class Propagator(object):
    ...
    def _calc_g_zs(self, g_z, c_i, g_zs):
        if USE_NUMBA:
            _calc_g_zs_numba(g_z, c_i, g_zs, LAMBDA_0, LAMBDA_1)
        else:
            pg_zs = g_zs[:, 0]
            segment_iterator = enumerate(c_i[::-1])
            next(segment_iterator)
            for r, c in segment_iterator:
                g_zs[:, r] = pg_zs = (correlate(pg_zs, LAMBDA_ARRAY) + c) * g_z

    def _calc_g_zs_uniform(self, g_z, g_zs):
        if USE_NUMBA:
            _calc_g_zs_uniform_numba(g_z, g_zs, LAMBDA_0, LAMBDA_1)
        else:
            segments = g_zs.shape[1]
            pg_zs = g_zs[:, 0]
            for r in range(1, segments):
                g_zs[:, r] = pg_zs = correlate(pg_zs, LAMBDA_ARRAY) * g_z

if USE_NUMBA:
    @njit('(f8[:], f8[:, :], f8, f8)', cache=True)
    def _calc_g_zs_uniform_numba(g_z, g_zs, f0, f1):
    ...
    def _calc_g_zs_numba(g_z, c_i, g_zs, f0, f1):
        points, segments = g_zs.shape
        for r in range(segments-1):
